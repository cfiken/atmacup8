...
class NameBOW3(Feature):
    ...
    def __init__(
        ...
        super().__init__(name='name_bow3', train=train)
    ...
    def create(
        ...
        *args, **kwargs
        ...
        processed_name_platform_whole = [n + ' ' + p for n,
                                         p in zip(df_whole.loc[:, 'processed_name'], df_whole.loc[:, 'Platform'])]
        df_whole.loc[:, 'processed_name_platform'] = processed_name_platform_whole
        processed_name_platform_main = [n + ' ' + p for n,
                                        p in zip(df_main.loc[:, 'processed_name'], df_main.loc[:, 'Platform'])]
        df_main.loc[:, 'processed_name_platform'] = processed_name_platform_main

        unique_name = pd.DataFrame(df_whole['processed_name_platform'].unique(), columns=['processed_name_platform'])

        bow1 = self._get_bow(unique_name.loc[:, 'processed_name_platform'].values,
                             (1, 1), self.word_count_th1, self.threshold_upper1)
        bow2 = self._get_bow(unique_name.loc[:, 'processed_name_platform'].values,
        ...
        df_main = pd.merge(df_main, unique_name, how='left', on='processed_name_platform')
