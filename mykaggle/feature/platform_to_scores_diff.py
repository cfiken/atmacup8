...
class PlatformToScoresDiff(Feature):
    '''
    Platform についてUserやCriticsのスコア、数の情報
    '''

    def __init__(self, train: bool = True, n_components: int = 2) -> None:
        super().__init__(name='platform_to_scores_diff', train=train)
    ...
    def create(
        ...
        *args, **kwargs
        ...
        transform = BasicGroupByTransform(keys=['Platform'], targets=TARGET_COLUMNS, aggs=['mean', 'max', 'min'])
        platform_scores = transform(df_whole)
        df_main = pd.merge(df_main, platform_scores, how='left', on='Platform')
